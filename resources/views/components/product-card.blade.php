...
<div x-data="toast" class="relative m-10 flex w-full max-w-xs flex-col overflow-hidden rounded-lg border border-gray-100 bg-white shadow-md">
  ...
  <div x-data="cart({{ $id }})" class="mt-4 px-5 pb-5">
    ...
    <div @click="store().then(b => b && show('{{ __('cart.added_to_cart') }}'))" class="btn bg-slate-900 flex items-center justify-center rounded-md  px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-gray-700 focus:outline-none focus:ring-4 focus:ring-blue-300">
